...
csvs2 = [f for f in os.listdir('ml2-master2/data/fa15/op+context-count+type+size') if f.endswith('.csv')]
csvs = [f for f in os.listdir('ml2-master2/data/sp14/op+context-count+type+size') if f.endswith('.csv')]
...
for csv in csvs:
	df, fs, ls = input.load_csv(os.path.join('ml2-master2/data/sp14/op+context-count+type+size', csv), filter_no_labels=True, only_slice=False)
...
for csv in csvs2:
	df2, fs2, ls2 = input.load_csv(os.path.join('ml2-master2/data/fa15/op+context-count+type+size', csv), filter_no_labels=True, only_slice=False)
